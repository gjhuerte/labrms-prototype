...
class AccountsController extends \BaseController {
	...
	public function update($id)
	{
		...
		return Redirect::to('account/view/update');
	}
	...
	/**
	 * Returns all active or inactive account based on given id
	 *
	 *@param  int  $id
	 * @return Response
	 */
	...
	public function retrieveInactiveAccount()
	{
		if(Request::ajax())
		{
			...
			$user = User::where("status","=",$status)->where('accesslevel','!=','0')->select('username','firstname','lastname','middlename','type','accesslevel','created_at','id')->get();
			return json_encode($user);
		}

		$user = User::where('status','=','0')->where('accesslevel','!=','0')->get();
	...
	public function activateAccount()
	{
		if(Request::ajax())
		{
			...
			$user = User::find($id);
			if($type == 'activate') $user->status = 1;
			else if($type == 'deactivate') $user->status = 0;
			$user->save();
	...
	/**
	 * Returns view for account update
	 *
	 * @return Response
	 */
	...
	/**
	 * Returns View for account deletion
	 *
	 * @return Response
	 */
