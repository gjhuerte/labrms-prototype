...
@include('modal.equipment.support.create')
@include('modal.equipment.support.edit')
		...
		<div class="panel panel-body   table-responsive">
			<table class="table table-hover table-striped table-bordered" id="equipmentSupportTable">
				<thead>
					<th>ID</th>
					<th>Type</th>
					<th id="problem">Problem Suggested / Category</th>
					<th>Created At</th>
					<th>Updated At</th>
				</thead>
			</table>
				...
				if(table.row('.selected').data().id != null && table.row('.selected').data().id  && table.row('.selected').data().id >= 0)
				{
			        ...
			        function(isConfirm){
			          if (isConfirm) {
     					$.ajax({
							type: 'delete',
							url: '{{ url("equipment/support/") }}' + "/" + table.row('.selected').data().id,
							data: {
