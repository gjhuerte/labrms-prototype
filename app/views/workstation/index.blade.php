...
<style>
	#page-body,#deploy,#transfer,#delete{
		display: none;
	}
</style>
@stop
	...
	$(document).ready(function() {
    	...
    	var table = $('#workstationTable').DataTable( {
	 	...
	 	$("div.toolbar").html(`
	 			...
	 			<button id="deploy" class="btn btn-default btn-flat" style="margin-right:5px;padding: 5px 10px;">
	    ...
	    $('#delete').on('click',function(){
			try{
				if(table.row('.selected').data().id != null && table.row('.selected').data().id  && table.row('.selected').data().id >= 0)
				{
			        swal({
			          title: "Are you sure?",
			          text: "This will condemn the workstation and all the associated equipments with it. Do you want to continue?",
			          type: "warning",
