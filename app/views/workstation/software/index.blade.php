				...
				<table class="table table-hover table-striped table-bordered" id="workstationTable">
					...
					<tbody>
						...
						<tr>
								...
								<td><a role="button" class="text-primary link" data-id="{{ $workstation->id }}">View Software Installed</a></td>
								<td><a role="button" class="text-primary link" data-id="{{ $workstation->id }}">View Location Deployed</a></td>
								<td>
									{{ Form::open() }}
	...
	$(document).ready(function() {
		...
		$('.link').on('click',function(){
			...
			$.ajax({
				...
				dataType: 'html',
				beforeSend: function(){
					swal('Requesting for information...','Please wait for a moment while softwares are being loaded');
				},
