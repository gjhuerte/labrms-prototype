...
<div class="container-fluid">
	<div class="col-sm-3" id="accordion" role="tablist" aria-multiselectable="true">
		<div class="panel panel-info">
			...
			@foreach($reservations as $reservation)
				@if(count($reservation->itemprofile) > 0 && count($reservation->user) > 0)
			    <a href="{{ route('reservation.show',$reservation->id) }}" class="list-group-item">
			      ...
			      <p class="list-group-item-text">Borrower: {{{ User::find($reservation->user_id)->first()->firstname }}} {{{ User::find($reservation->user_id)->first()->lastname }}}</p>

			      <p class="list-group-item-text">Item Name: {{{ Itemprofile::find($reservation->item_id)->first()->inventory->itemname }}} </p>

			      <p class="list-group-item-text">Property Number: {{{ Itemprofile::find($reservation->item_id)->first()->property_number }}} </p>
	...
	<div class=" col-md-6 col-sm-12">
		<div class="col-md-6 col-sm-12 panel panel-body" style="background-color:white; padding: 10px;">
			@if(count($roomlogs) == 0)
			<div class="col-sm-12 text-danger">
				<strong>{{ Carbon\Carbon::now()->toFormattedDateString() }} </strong><br/>
				<p> No room log available</p>
			</div>
			<div class="col-sm-12">
				{{ Form::open(['method'=> 'get','route' => 'room.log.create']) }}
				{{ Form::submit('Create',[
			...
			<div class="col-sm-12 text-success">
				<strong>{{ Carbon\Carbon::now()->toFormattedDateString() }} </strong><br/>
				<p> Room log available!</p>
			</div>
			<div class="col-sm-12">
				{{ Form::open(['method'=> 'get','route' => 'room.log.create']) }}
				{{ Form::submit('Create more',[
					'class' => 'btn btn-primary btn-block'
				]) }}
				{{ Form::close() }}
			</div>
		...
		<div class="col-sm-12 panel panel-body"  id='calendar'></div>
		@if(count($roomlogs) > 0)
			@foreach($roomlogs as $log)
			<div class="col-sm-12 panel panel-body" style="background-color:white; padding: 10px;">
				<div class="col-sm-8">
					<legend><h3 class="text-primary">Room Name:{{ $log->room->name }}</h3> </legend>
					<p>Faculty-in-charge: {{ $log->facultyincharge }}</p>
					<p>Section: {{ $log->section }}</p>
					<p>Working Units: {{ $log->workingunits }}</p>	
					<p>Staff-in-charge: {{ $log->staffincharge }}</p>
					<p>Time-in: {{{ Carbon\Carbon::parse($log->timein)->toDayDateTimeString() }}}</p>
					<p>Time-out: {{{ Carbon\Carbon::parse($log->timeout)->toDayDateTimeString() }}}</p>
				</div>
			</div>
			@endforeach
	...
	<div class="col-sm-3" id="accordion" role="tablist" aria-multiselectable="true">
		<div class="panel panel-primary">
			...
			@foreach($tickets as $ticket)
			    ...
			    @elseif( count($ticket->itemprofile) > 0 )
			    <a href="{{ route('item.profile.show',$ticket->itemprofile->id) }}" class="list-group-item">
			    @else
			    <a href="#" class="list-group-item">
			    @endif
				@if($ticket->type == 'maintenance')
			      <h4 class="list-group-item-heading"><p class="text-warning">Maintenance Ticket</p></h4>
			    @elseif($ticket->type == 'receive')
			      <h4 class="list-group-item-heading"><p class="text-success">Received Ticket</p></h4>
			    @elseif($ticket->type == 'complaint')
			      <h4 class="list-group-item-heading">
			      	<p class="text-danger">Complaint Ticket</p>
			      	<span class="label label-info">Action taken</span>
			      </h4>
			    @elseif($ticket->type == 'incident')
			      <h4 class="list-group-item-heading">
			      	<p class="text-danger">Incident Ticket
				  	@if(count($ticket->actiontaken) > 0)
			    ...
			    @else
			      <h4 class="list-group-item-heading"><p class="text-info">Condemn Ticket</p></h4>
			    @endif

				  <p class="list-group-item-text">Title: {{{ $ticket->title }}}</p>
				  <p class="list-group-item-text">Description: {{{ $ticket->description }}}</p>
				  @if( count($ticket->user) > 0)
				  <p class="list-group-item-text">Author: {{{ $ticket->user->firstname }}} {{{ $ticket->user->lastname }}}</p>
				  @endif
				  @if( count($ticket->itemprofile) > 0)
				  <p class="list-group-item-text">Item Name: {{{ $ticket->itemprofile->inventory->itemname }}} </p>
				  <p class="list-group-item-text">Property Number: {{{ $ticket->itemprofile->property_number }}} </p>
				  @endif
			    </a>
			@endforeach
			</div>
