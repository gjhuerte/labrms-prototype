	...
	$(document).ready(function(){
	    var table = $('#softwareTable').DataTable( {
	 	...
	 	$("div.toolbar").html(`
 			...
 			<button id="assign" class="btn btn-warning btn-flat" style="margin-right:5px;padding: 5px 10px;"><span class="glyphicon"></span> Assign</button>
		`);
		...
		$('#assign').on('click',function(){
			try{
				if(table.row('.selected').data().id != null && table.row('.selected').data().id  && table.row('.selected').data().id >= 0)
				{
					$('#assign-id').val(table.row('.selected').data().id)
