...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("div.toolbar").html(`
				<div class="btn-group">
							<button id="edit" class="btn btn-default btn-flat" type="button" style="padding:10px">
								<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
								<span class="hidden-sm hidden-xs">Update</span>
							</button>
				</div>
				<div class="btn-group">
	    ...
	    $('#delete').on('click',function(){
			try{
				if(table.row('.selected').data().id != null && table.row('.selected').data().id  && table.row('.selected').data().id >= 0)
				{
			        swal({
			          title: "Are you sure?",
			          text: "This item will be removed from database?",
			          type: "warning",
